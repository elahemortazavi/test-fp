...
const Attendance = () => {
  ...
  // State for your existing variables
  ...
  // Existing useEffect for fetching attendance data
  useEffect(() => {
    fetchAttendanceData();
  }, []);

  // Existing fetchAttendanceData function
  ...
  // Existing handleViewAttendeesByDate function
 const handleViewAttendeesByDate = async () => {
  ...
  // Existing JSX for rendering attendees by date
  const renderAttendeesByDate = () => {
    return (
  ...
  // Existing JSX for rendering breakout rooms
 const renderBreakoutRooms = () => {
   return (
     <div className="breakout-rooms">
       {breakoutRooms.map((room, roomIndex) => (
         <div className="breakout-room" key={`room-${roomIndex}`}>
           <h3>Breakout Room {roomIndex + 1}</h3>
           <div className="breakout-persons">
             <div className="in-person">
               <h4>In-Person</h4>
               <ul>
                 {room.inPerson.map((person, personIndex) => (
                   <li key={`inPerson-${roomIndex}-${personIndex}`}>
                     {person.name}
                   </li>
                 ))}
               </ul>
             ...
             <div className="online">
               <h4>Online</h4>
               <ul>
                 {room.online.map((person, personIndex) => (
                   <li key={`online-${roomIndex}-${personIndex}`}>
                     {person.name}
                   </li>
                 ))}
               </ul>
...
//       <div className="navbar">{/* ... (Existing navigation bar JSX) */}</div>
//       <div className="middle-container">{/* ... (Existing form JSX) */}</div>
//       <div className="container">{/* ... (Existing attendees list JSX) */}</div>
//       <div className="datepicker-container">
//         {/* ... (Existing date picker JSX) */}
//       </div>
//       {/* Render the breakout rooms */}
//       {showBreakoutRooms && renderBreakoutRooms()}
//     </div>
//   );
...
return (
  <div className="top-container">
      ...
      <form className="middle-container" onSubmit={handleSubmit}>
        ...
        <div className="attendance-select">
          <select
            ...
            className={`select-container ${attendanceTypeError ? "error" : ""}`}
            id="attendanceType"
    ...
    {/* Render the breakout rooms */}
    <div className="breakout-rooms-button">
      <button onClick={() => setShowBreakoutRooms(!showBreakoutRooms)}>
        {showBreakoutRooms ? "Hide Breakout Rooms" : "Show Breakout Rooms"}
      </button>
    </div>
    {showBreakoutRooms && renderBreakoutRooms()}
  </div>
